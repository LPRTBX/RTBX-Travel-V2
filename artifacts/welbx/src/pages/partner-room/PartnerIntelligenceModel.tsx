...
const C = {
  gold: "#c9a84c", muted: "rgba(255,255,255,0.5)", dim: "rgba(255,255,255,0.22)",
    ...
    <PartnerRoomLayout>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "72px 32px 140px" }}>
        ...
        <div style={{ padding: "18px 22px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderLeft: "3px solid #c9a84c", marginBottom: 40 }}>
          <p style={{ fontSize: 13.5, color: "rgba(255,255,255,0.72)", lineHeight: 1.75, margin: 0, fontWeight: 600 }}>
            ...
            {SYSTEMS_FLOW.map((step, i, arr) => (
              <div key={step} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{
                  ...
                  color: i === arr.length - 1 ? "#080c14" : "rgba(255,255,255,0.6)",
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          {domains.map(domain => (
            <div key={domain} style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: DOMAIN_COLOR[domain], letterSpacing: "0.04em", marginBottom: 8 }}>{domain}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {TRAVEL_SIGNAL_TAXONOMY.filter(s => s.domain === domain).map(s => (
                  <div key={s.id} title={s.description} style={{
                    padding: "6px 11px", fontSize: 10.5, color: "rgba(255,255,255,0.6)",
            ...
            {TRAVEL_MOMENT_TAXONOMY.map(m => (
              <div key={m.id} style={{ padding: "18px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderTop: `2px solid ${RISK_COLOR[m.riskLevel]}` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, gap: 8 }}>
                  ...
                  <div style={{ fontSize: 8.5, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: RISK_COLOR[m.riskLevel], flexShrink: 0 }}>{m.riskLevel}</div>
                </div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.4)", marginBottom: 10 }}>{m.valueAtStake}</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 10, color: "rgba(255,255,255,0.5)", marginBottom: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 1 }}>
            {TRAVEL_GOVERNANCE_SOURCES.map(g => (
              <div key={g.id} style={{ padding: "14px 18px", background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: "#fff", marginBottom: 4 }}>{g.name}</div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>{g.description}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                  {["Role", "Sees", "Can do", "Cannot do", "Owns", "Can approve", "Escalation trigger"].map(h => (
                    <th key={h} style={{ textAlign: "left", padding: "8px 10px", color: "rgba(255,255,255,0.35)", fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", fontSize: 8.5 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {TRAVEL_ROLE_MODEL.map(r => (
                  <tr key={r.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.55)" }}>{r.sees}</td>
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.55)" }}>{r.canDo}</td>
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.4)" }}>{r.cannotDo}</td>
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.55)" }}>{r.owns}</td>
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.55)" }}>{r.canApprove}</td>
                    <td style={{ padding: "10px", color: "rgba(255,255,255,0.4)" }}>{r.triggersEscalation}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, marginBottom: 20 }}>
            <div style={{ padding: "20px 22px", background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.18)" }}>
              <div style={{ fontSize: 9, letterSpacing: "0.12em", color: C.green, textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>AI May</div>
              {TRAVEL_AI_ASSISTANT_MODEL.allowed.map(item => (
                <div key={item} style={{ display: "flex", gap: 10, marginBottom: 8, fontSize: 12, color: "rgba(255,255,255,0.7)" }}>
            ...
            <div style={{ padding: "20px 22px", background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.18)" }}>
              <div style={{ fontSize: 9, letterSpacing: "0.12em", color: C.red, textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>Requires Human Approval</div>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {TRAVEL_OUTCOME_MODEL.map(o => (
              <div key={o.id} title={o.description} style={{ padding: "8px 13px", fontSize: 11, color: "rgba(255,255,255,0.62)", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)" }}>
                {o.name}
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 2 }}>
            {TRAVEL_VALUE_MODEL.map(cat => (
              <div key={cat.id} style={{ padding: "18px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ fontSize: 10.5, fontWeight: 800, color: C.gold, marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>{cat.label}</div>
                {cat.measures.map(m => (
                  <div key={m.id} style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", marginBottom: 7 }}>{m.name}</div>
                ))}
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 1 }}>
            {TRAVEL_INTEGRATION_MAP.map(i => (
              <div key={i.id} style={{ padding: "14px 18px", background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: "#fff", marginBottom: 4 }}>{i.name}</div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>{i.description}</div>
              </div>
            ))}
          </div>
        </div>
