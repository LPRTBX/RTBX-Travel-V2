...
const C = { muted: "rgba(255,255,255,0.5)", dim: "rgba(255,255,255,0.22)", gold: "#c9a84c", green: "#10b981", blue: "#3b82f6", red: "#ef4444", purple: "#a78bfa" };
...
function MTag({ status }: { status: PartnerMaturityStatus }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", padding: "3px 9px", fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", color: MATURITY_COLORS[status], border: `1px solid ${MATURITY_COLORS[status]}50` }}>
      ...
      <details open style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderLeft: `3px solid ${color}` }}>
        <summary style={{ padding: "18px 22px", cursor: "pointer", listStyle: "none", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 10.5, color: C.muted, maxWidth: 680 }}>{lane.summary.split(".")[0]}.</div>
          </div>
        ...
        <div style={{ padding: "0 22px 22px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px 16px", marginBottom: 16 }}>
            {[
              ...
              <div key={col.title} style={{ padding: "12px 14px", background: "rgba(255,255,255,0.02)", border: `1px solid ${col.c}20` }}>
                <div style={{ fontSize: 8.5, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: col.c, marginBottom: 8 }}>{col.title}</div>
                {col.items.map((item, i) => (
                  <div key={i} style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", padding: "3px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", lineHeight: 1.5 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px", marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 8.5, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700, marginBottom: 6 }}>Partner types</div>
              {lane.partnerTypes.map((t, i) => <div key={i} style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", padding: "2px 0" }}>· {t}</div>)}
            </div>
            <div>
              <div style={{ fontSize: 8.5, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700, marginBottom: 6 }}>Commercial models (indicative)</div>
              {lane.commercialModels.map((m, i) => <div key={i} style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", padding: "2px 0" }}>· {m}</div>)}
            </div>
          </div>

          <div style={{ padding: "8px 12px", background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 9.5, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>
            {lane.maturityNote}
          </div>
        </div>
      </details>
    ...
    <PartnerRoomLayout>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "72px 32px 140px" }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.8, maxWidth: 700, marginBottom: 20 }}>
          ...
          <div style={{ padding: "12px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 11, color: "rgba(255,255,255,0.4)", fontStyle: "italic" }}>
        ...
        <div id="why-ecosystem" style={{ marginBottom: 56, scrollMarginTop: 90 }}>
          ...
          <p style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.75, maxWidth: 760, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2, marginBottom: 16 }}>
            {[
              ...
              <div key={item.label} style={{ padding: "16px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderTop: `2px solid ${item.color}` }}>
                <div style={{ fontSize: 11.5, fontWeight: 800, color: "#fff", marginBottom: 6 }}>{item.label}</div>
        ...
        <div id="rtbx-ownership" style={{ marginBottom: 48, scrollMarginTop: 90 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2 }}>
            {RTBX_OWNED_CAPABILITIES.map((cap, i) => (
              <div key={i} style={{ padding: "10px 14px", background: "rgba(201,168,76,0.04)", border: "1px solid rgba(201,168,76,0.12)", fontSize: 10.5, color: "rgba(255,255,255,0.7)" }}>
        ...
        <div id="customer-ownership" style={{ marginBottom: 48, scrollMarginTop: 90 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2 }}>
            {CUSTOMER_OWNED_CAPABILITIES.map((cap, i) => (
              <div key={i} style={{ padding: "10px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 10.5, color: "rgba(255,255,255,0.65)" }}>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                  {["Partner type", "Partner owns", "RTBX owns", "Customer owns"].map(h => (
                    <th key={h} style={{ textAlign: "left", padding: "8px 14px", color: "rgba(255,255,255,0.35)", fontWeight: 700, fontSize: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {PARTNER_OWNERSHIP_MATRIX.map((row, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "10px 14px", color: "rgba(255,255,255,0.5)" }}>{row.customerOwns}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div id="integration-responsibility" style={{ marginBottom: 56, scrollMarginTop: 90 }}>
          ...
          <p style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.65, maxWidth: 760, marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.3)", fontStyle: "italic", marginBottom: 16 }}>
            ...
            {INTEGRATION_RECORDS.map(rec => (
              <details key={rec.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderLeft: `3px solid ${INTEGRATION_MATURITY_COLORS[rec.maturity]}` }}>
                <summary style={{ padding: "12px 18px", cursor: "pointer", listStyle: "none", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                  <div>
                    <span style={{ fontSize: 12, fontWeight: 800, color: "#fff", marginRight: 12 }}>{rec.system}</span>
                    {rec.provider && <span style={{ fontSize: 9.5, color: "rgba(255,255,255,0.4)" }}>{rec.provider}</span>}
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <div style={{ padding: "2px 8px", fontSize: 8.5, fontWeight: 700, color: INTEGRATION_MATURITY_COLORS[rec.maturity], border: `1px solid ${INTEGRATION_MATURITY_COLORS[rec.maturity]}40` }}>
                ...
                <div style={{ padding: "0 18px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 24px" }}>
                  <div style={{ gridColumn: "1/-1", fontSize: 10.5, color: C.muted, marginBottom: 8 }}>{rec.purpose}</div>
                  {[
                    ...
                    <div key={label} style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", lineHeight: 1.55 }}>
                      <span style={{ color: "rgba(255,255,255,0.3)", fontWeight: 700 }}>{label}: </span>{val}
                    </div>
                  ))}
                  {rec.maturityNote && (
                    <div style={{ gridColumn: "1/-1", marginTop: 6, padding: "8px 12px", background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 9.5, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>
                      {rec.maturityNote}
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {DEPLOYMENT_RESPONSIBILITIES.map(act => (
              <details key={act.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <summary style={{ padding: "11px 18px", cursor: "pointer", listStyle: "none", fontSize: 11.5, fontWeight: 700, color: "#fff" }}>{act.label}</summary>
                <div style={{ padding: "0 18px 14px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 20px" }}>
                  {[
                    ...
                    <div key={String(label)} style={{ fontSize: 10.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
            {PARTNER_COMMERCIAL_MODELS.map(model => (
              <div key={model.id} style={{ padding: "18px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 6 }}>{model.partnerType}</div>
                ...
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", fontStyle: "italic" }}>{model.statusNote}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 2 }}>
            {PARTNER_SELECTION_CRITERIA.map(cat => (
              <div key={cat.id} style={{ padding: "18px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: C.gold, marginBottom: 10, letterSpacing: "0.02em" }}>{cat.label}</div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
            {PARTNERSHIP_PATHWAY_STAGES.map((stage, i) => (
              <div key={stage.id} style={{ flex: "1 1 180px", padding: "16px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderTop: `2px solid ${i === 0 ? C.gold : "rgba(255,255,255,0.12)"}` }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: "#fff", marginBottom: 6 }}>{stage.label}</div>
                <div style={{ fontSize: 10, color: C.muted, marginBottom: 8, lineHeight: 1.5 }}>{stage.description}</div>
                {stage.actions.map((a, j) => <div key={j} style={{ fontSize: 9.5, color: "rgba(255,255,255,0.45)", padding: "2px 0" }}>· {a}</div>)}
              </div>
            ))}
          </div>
        </div>
        ...
        <div id="next-step" style={{ marginBottom: 40, scrollMarginTop: 90 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <Link href="/partner-room/next-step">
              <div style={{ padding: "12px 22px", background: C.gold, fontSize: 11, fontWeight: 700, color: "#080c14", cursor: "pointer" }}>Explore a Partner Model →</div>
            </Link>
            <Link href="/partner-room/next-step#responsibilities">
              <div style={{ padding: "12px 22px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.15)", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.7)", cursor: "pointer" }}>Review Responsibilities →</div>
            </Link>
          </div>
        </div>
