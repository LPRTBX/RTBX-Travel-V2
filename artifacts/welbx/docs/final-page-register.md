# RTBX Travel Partner Room — Final Page Register

**Document type:** Navigation audit reference  
**Owner:** RTBX  
**Version:** Sprint 5 final  
**Created by:** Task #18 (Navigation, Routes & Brand Cleanup) — validated by Task #22  
**Last validated:** July 2025  

---

## Purpose

This register lists every canonical route in the RTBX Travel Partner Room, its component, its primary section, its audience, and its navigation status. It is the source of truth for navigation audits and link validation.

---

## Primary Partner Room Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 1 | `/partner-room` | PartnerRoomLanding | Entry | All partners | Canonical |
| 2 | `/partner-room/overview` | PartnerOverview | Overview | All partners | Canonical |
| 3 | `/partner-room/deployments` | PartnerDeployments | Deployments | All partners | Canonical |
| 4 | `/partner-room/product-proof` | PartnerProductProof | Product Proof | All partners | Canonical |
| 5 | `/partner-room/validation` | PartnerValidation | Product Proof | Technical partners | Canonical |
| 6 | `/partner-room/commercial` | PartnerCommercial | Commercial | Commercial leads | Canonical |
| 7 | `/partner-room/brief-library` | PartnerBriefLibrary | Brief Library | All partners | Canonical |
| 8 | `/partner-room/next-step` | PartnerNextStep | Next Step | All partners | Canonical |
| 9 | `/partner-room/operator-brief` | PartnerOperatorBrief | Operator Brief | Operators | Canonical |
| 10 | `/partner-room/integration-brief` | PartnerIntegrationBrief | Integration Brief | Technology partners | Canonical |
| 11 | `/partner-room/moments-economy` | PartnerMomentsEconomy | Architecture | All partners | Canonical |
| 12 | `/partner-room/signals-engine` | PartnerSignalsEngine | Architecture | Technical | Canonical |
| 13 | `/partner-room/pilot-model` | PartnerPilotModel | Pilot | All partners | Canonical |
| 14 | `/partner-room/commercial-model` | PartnerCommercialModel | Commercial | Commercial leads | Canonical |
| 15 | `/partner-room/demo-paths` | PartnerDemoPaths | Demos | All partners | Canonical |
| 16 | `/partner-room/live-demos` | PartnerLiveDemos | Demos | All partners | Canonical |

---

## Execution Centre Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 17 | `/partner-room/guest-demo` | PartnerGuestDemo | Execution Centre | All partners | Canonical |
| 18 | `/partner-room/operator-demo` | PartnerOperatorDemo | Execution Centre | Operators | Canonical |
| 19 | `/partner-room/dual-view-demo` | PartnerDualViewDemo | Execution Centre | All partners | Canonical |
| 20 | `/partner-room/build-configure` | PartnerBuildConfigure | Execution Centre | All partners | Canonical |
| 21 | `/partner-room/operations` | PartnerOperationsCentre | Execution Centre | Operators | Canonical |
| 22 | `/partner-room/travel-scenarios` | PartnerTravelScenarios | Execution Centre | All partners | Canonical |
| 23 | `/travel-scenarios` | PartnerTravelScenarios | — | — | Alias → canonical |
| 24 | `/partner-room/comms-demo` | PartnerCommsDemo | Execution Centre | All partners | Canonical |
| 25 | `/partner-room/travel-ai-comms` | PartnerTravelAiComms | Execution Centre | All partners | Canonical |
| 26 | `/travel-ai-comms` | PartnerTravelAiComms | — | — | Alias → canonical |

---

## Intelligence & Architecture Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 27 | `/partner-room/operating-model` | PartnerOperatingModel | Architecture | All partners | Canonical |
| 28 | `/partner-room/intelligence-model` | PartnerIntelligenceModel | Architecture | Technical | Canonical |
| 29 | `/partner-room/travel-intelligence` | PartnerIntelligenceModel | — | — | Alias → canonical |
| 30 | `/travel-intelligence` | PartnerIntelligenceModel | — | — | Alias → canonical |
| 31 | `/partner-room/travel-operating-systems` | PartnerTravelOperatingSystems | Architecture | All partners | Canonical |
| 32 | `/travel-operating-systems` | PartnerTravelOperatingSystems | — | — | Alias → canonical |
| 33 | `/partner-room/decision-spine` | PartnerDecisionSpine | Architecture | Technical | Canonical |
| 34 | `/partner-room/scenario-builder` | PartnerScenarioBuilder | Product Proof | Technical | Canonical |
| 35 | `/partner-room/proof-calculator` | PartnerProofCalculator | Commercial | Commercial leads | Canonical |

---

## Deployment & Partner Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 36 | `/partner-room/partner-ecosystem` | PartnerEcosystem | Partner Ecosystem | All partners | Canonical |
| 37 | `/partner-room/rollout-model` | PartnerRolloutModel | Deployment | All partners | Canonical |
| 38 | `/partner-room/commercial-unit` | PartnerCommercialUnit | Commercial | Commercial leads | Canonical |

---

## Deployment Vertical Demo Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 39 | `/partner-room/deployments/hotels-resorts/demo` | PartnerHotelsResortsDemo | Deployments | Hotels | Canonical |
| 40 | `/partner-room/deployments/corporate-travel/demo` | PartnerCorporateTravelDemo | Deployments | Corporate | Canonical |
| 41 | `/partner-room/deployments/events-venues/demo` | PartnerEventsVenuesDemo | Deployments | Events | Canonical |
| 42 | `/partner-room/deployments/destination-tourism/demo` | PartnerDestinationTourismDemo | Deployments | Tourism | Canonical |
| 43 | `/partner-room/holiday-park-demo` | PartnerHolidayParkDemo | Deployments | Holiday Parks | Canonical |

---

## Product Proof Deep-Dive Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 44 | `/partner-room/product-proof/signal-capture` | PartnerSignalCapture | Product Proof | Technical | Canonical |
| 45 | `/partner-room/product-proof/stage-3-operating-layer` | PartnerStage3Preview | Product Proof | Technical | Canonical |
| 46 | `/partner-room/product-proof/pilot-expansion-preview` | PartnerPilotExpansionPreview | Product Proof | All partners | Canonical |
| 47 | `/partner-room/validation-replay` | PartnerValidationReplay | Product Proof | Technical | Canonical |

---

## Redirect Routes

| # | Route | Redirects To | Reason |
|---|---|---|---|
| 48 | `/travel-action-centre` | `/partner-room/operations#action-centre` | Legacy URL compatibility |
| 49 | `/travel-outcomes` | `/partner-room/operations#outcome-ledger` | Legacy URL compatibility |
| 50 | `/travel-value` | `/partner-room/operations#value-dashboard` | Legacy URL compatibility |
| 51 | `/story/executive-briefing` | `/story` | Deprecated story route |
| 52 | `*` (catch-all) | `/partner-room` | Default redirect |

---

## Story / Operator Story Lab Routes

| # | Route | Component | Section | Audience | Status |
|---|---|---|---|---|---|
| 53 | `/story` | StoryHub | Story Lab | Executive sponsors | Canonical |
| 54 | `/story/operator-deep-dive` | StoryOperator | Story Lab | Operators | Canonical |
| 55 | `/story/live-guest-story` | StoryGuestStory | Story Lab | All partners | Canonical |

---

## Resource / Appendix Routes

These routes are accessible from the Brief Library. They contain appendix documents and are not in the primary sidebar navigation.

| # | Route | Component | Audience | Confidentiality | Status |
|---|---|---|---|---|---|
| 56 | `/partner-room/resources/travel-partnership-overview` | TravelPartnershipOverview | All partners | General partner | Canonical |
| 57 | `/partner-room/resources/travel-commercial-partnership-brief` | TravelCommercialPartnershipBrief | Commercial | Commercially restricted | Canonical |
| 58 | `/partner-room/resources/travel-business-plan` | TravelBusinessPlan | Executive sponsors | General partner | Canonical |
| 59 | `/partner-room/resources/travel-gtm-plan` | TravelGtmPlan | Commercial | General partner | Canonical |
| 60 | `/partner-room/resources/travel-commercial-case` | TravelCommercialCase | Commercial | Commercially restricted | Canonical |
| 61 | `/partner-room/resources/travel-ux-blueprint` | TravelUxBlueprint | Technical | Technical partner | Canonical |
| 62 | `/partner-room/resources/travel-systems-map` | TravelSystemsMap | Technical | Technical partner | Canonical |
| 63 | `/partner-room/resources/travel-pilot-model` | TravelPilotModel | All partners | Pilot partner | Canonical |
| 64 | `/partner-room/resources/travel-revenue-model` | TravelRevenueModel | Commercial | Commercially restricted | Canonical |
| 65 | `/partner-room/resources/travel-demo-links` | TravelDemoLinks | All partners | General partner | Canonical |
| 66 | `/partner-room/resources/travel-ai-intelligence-layer` | TravelAiIntelligenceLayer | Technical | Technical partner | Canonical |
| 67 | `/partner-room/resources/travel-architecture-modelling-ux-qa` | TravelArchitectureModellingUxQa | Technical | Technical partner | Canonical |

---

## Navigation Audit Summary

| Category | Count |
|---|---|
| Canonical partner room routes | 42 |
| Alias routes (same component) | 7 |
| Redirect routes | 5 |
| Story lab routes | 3 |
| Resource/appendix routes | 12 |
| **Total registered routes** | **69** |

---

## Navigation Validation Results

| Check | Result |
|---|---|
| All routes in PARTNER_ROUTES resolved by `test-routes.mjs` | ✅ 57 imports, 0 missing |
| All internal links resolved by `test-links.mjs` | ✅ 59 links, 0 broken |
| No legacy routes reachable (BXOS, NEXUS) | ✅ check:legacy passed |
| TypeScript typecheck | ✅ 0 errors |
| Build | ✅ No errors |
